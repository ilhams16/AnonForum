	...
	public class PostBLL : IPostBLL
	{
		...
		public PostDTO GetPostbyID(int postID)
		{
			...
			if (post != null)
			{
				...
				// Populate the list with CommentDTO objects
				...
				postDto.Category = new PostCategoryDTO()
				{
					PostCategoryID = post.Category.PostCategoryID,
					Name = post.Category.Name,
				};
				postDto.UserPost = new UserDTO()
				{
					UserID = post.UserPost.UserID,
					Username = post.UserPost.Username,
					UserImage = post.UserPost.UserImage,
				};
			}
			else
			{
		...
		public IEnumerable<PostDTO> GetAllPosts()
		{
			...
			foreach (var post in posts)
			{
				listPostsDto.Add(new PostDTO
				{
					...
					UserPost = new UserDTO()
					{
						UserID = post.UserPost.UserID,
						Username = post.UserPost.Username,
						UserImage = post.UserPost.UserImage,
					},
					Likes = (List<int>)(_postDAL.GetUserLike(post.PostID)),
					Dislikes = (List<int>)(_postDAL.GetUserDislike(post.PostID))
				}); ;
			}
			return listPostsDto;
		}
		...
		public void EditPost(EditPostDTO entity)
		{
			if (string.IsNullOrEmpty(entity.Title))
			{
				throw new ArgumentException("Title is required");
			...
			try
			{
				...
				_postDAL.EditPost(entity.PostID, newPost);
			}
			catch (Exception ex)
			{
				throw new ArgumentException(ex.Message);
